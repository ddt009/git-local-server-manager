...
ChangGitUserSShKey(){
    ...
    if [ -d "/home/${username}" ]; then
        ...
        ssh-keygen -t rsa -b 4096 -C "${username}@gt-users" -f ${workdir}/id_rsa -q -N ""
...
SetChrootEnv(){
    if [ ! -f "${DEFAULT_CHROOT_BASE}/usr/bin/git-shell" ]; then
        ...
        # 挂载登记进fstab
        CHECK_FSTAB_DATA=$(grep "${DEFAULT_CHROOT_BASE}/dev" /etc/fstab)
        if [[ -z $CHECK_FSTAB_DATA ]]; then
...
ChangeSshConfig(){
    ...
    if grep -q "^Match Group ${GIT_GROUP_NAME}" "${sshd_config}"; then
        ...
        echo "Adding group match in ssh_config"
        cat <<EOF >>${sshd_config}
